		...
		var Times = {
			...
			init: function() {
				...
				Times.$table.addEventListener("click", function(event) {
					...
					var settings = {title : "Delete Entry", size : blackberry.ui.dialog.SIZE_MEDIUM, position : blackberry.ui.dialog.LOC_BOTTOM};
						...
						if (op == "edit") {
							...
							blackberry.ui.dialog.standardAsk("Are You Sure You Want To Remove This Entry?", blackberry.ui.dialog.D_YES_NO, removeEntry(),settings);
...
$(function() {
  ...
  $(edit).blur(function() {
    localStorage.setItem('projectTitle', this.innerHTML);
  });
  
  // when the page loads
  if ( localStorage.getItem('projectTitle') ) {
    edit.innerHTML = localStorage.getItem('projectTitle');
  }
...
{
	var URL = new blackberry.invoke.BrowserArguments("http://www.twitter.com/jlewsader")
	
	blackberry.invoke.invoke(blackberry.invoke.APP_BROWSER, URL);
}

function openWeb()
{
	var URL = new blackberry.invoke.BrowserArguments("http://www.jlewsader.com")
...
{
	var ops = {title : "Clear Data", size : blackberry.ui.dialog.SIZE_MEDIUM, position : blackberry.ui.dialog.LOC_BOTTOM};
	
	blackberry.ui.dialog.standardAsk("Are You Sure You Want To Clear Data?", blackberry.ui.dialog.D_YES_NO, clearstorage(),ops);
}

// Background/Foreground Handles
function handleBackground()
{
	pauseApplication();
	saveApplicationState();		//Save application state to LocalStorage
}
function handleForeground()
{
	restoreApplicationState();  //Retrieve saved state info
	resumeApplication();
}
...
function exitApp()	
{
	try
	{
		if (confirm("This will exit the app"))
		{
		  blackberry.app.exit();
		}
	} 
	catch(e) {
		alert("Exception in exitApp: " + e);
	}
}
